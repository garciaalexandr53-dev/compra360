...
BEGIN
  ...
  totais AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE f.status_grupo = 'sim')::bigint AS total_sim,
      COUNT(*) FILTER (WHERE f.status_grupo = 'nao')::bigint AS total_nao,
      COUNT(*) FILTER (WHERE f.status_grupo = 'pendente')::bigint AS total_pendente
    FROM filtrado f
  )
  SELECT
