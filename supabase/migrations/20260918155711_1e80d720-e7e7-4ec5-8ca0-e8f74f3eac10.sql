...
DROP FUNCTION IF EXISTS public.admin_list_fornecedores(text, integer, integer);
DROP FUNCTION IF EXISTS public.admin_update_fornecedor(uuid, text, text, text, text, numeric, text, text);

CREATE FUNCTION public.admin_list_fornecedores(_search text DEFAULT NULL::text, _limit integer DEFAULT 50, _offset integer DEFAULT 0, _filtro text DEFAULT NULL::text)
...
CREATE FUNCTION public.admin_update_fornecedor(
  _fornecedor_id uuid,
  _nome text,
...
REVOKE EXECUTE ON FUNCTION public.admin_update_fornecedor(uuid, text, text, text, text, numeric, text, text, text, text[]) FROM anon;