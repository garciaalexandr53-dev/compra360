...
CREATE OR REPLACE FUNCTION public.admin_list_clientes()
 RETURNS TABLE(user_id uuid, email text, created_at timestamp with time zone, loja_principal text, cnpj text, whatsapp text, total_lojas bigint, total_produtos bigint, total_produtos_inativos bigint, total_fornecedores bigint, total_cotacoes bigint, total_pedidos bigint, plan_name text, plan_status text, trial_end timestamp with time zone, ultima_cotacao_at timestamp with time zone)
 LANGUAGE plpgsql
