...
CREATE FUNCTION public.admin_list_clientes()
 RETURNS TABLE(user_id uuid, email text, created_at timestamp with time zone, loja_principal text, cnpj text, whatsapp text, nome_contato text, total_lojas bigint, total_produtos bigint, total_produtos_inativos bigint, total_fornecedores bigint, total_cotacoes bigint, total_pedidos bigint, plan_name text, plan_status text, trial_end timestamp with time zone, ultima_cotacao_at timestamp with time zone)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    u.email::text,
    u.created_at,
    (SELECT l.nome FROM public.lojas l WHERE l.user_id = u.id ORDER BY l.created_at ASC LIMIT 1) AS loja_principal,
    (SELECT l.cnpj FROM public.lojas l WHERE l.user_id = u.id AND l.cnpj IS NOT NULL ORDER BY l.created_at ASC LIMIT 1) AS cnpj,
    (SELECT pr.whatsapp FROM public.profiles pr WHERE pr.user_id = u.id LIMIT 1) AS whatsapp,
    (SELECT pr.nome FROM public.profiles pr WHERE pr.user_id = u.id AND pr.nome IS NOT NULL AND btrim(pr.nome) <> '' LIMIT 1) AS nome_contato,
    (SELECT COUNT(*) FROM public.lojas l WHERE l.user_id = u.id) AS total_lojas,
    (SELECT COUNT(*) FROM public.produtos p WHERE p.user_id = u.id AND p.ativo = true) AS total_produtos,
