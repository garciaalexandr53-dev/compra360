...
Nome de exibição do cliente: `admin_list_clientes` devolve `nome_contato` (subconsulta em `profiles.nome`). Helpers `getNomeExibicao` (nome pessoal → empresa → e-mail) e `getPrimeiroNome` (saudações) em `adminHelpers.ts` são a fonte única — listagem, alertas, contatos, sheets e `getMensagem` usam sempre o nome da pessoa quando existe, com a empresa na linha secundária. `ClienteDetalhesSheet` separa "Responsável" e "Empresa".
